...
| envoy.resources | object | `{"limits":{"cpu":4,"memory":"4G"},"requests":{"cpu":2,"memory":"2G"}}` | Resource requests and limits for Envoy Proxy. Note: an Envoy Proxy with too many connections might run out of CPU |
