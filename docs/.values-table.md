...
| modelLoader.enabled | bool | `false` | Enable model loader that exposes Prometheus metrics for Triton model status |
| modelLoader.logLevel | string | `"INFO"` | Logging level (DEBUG, INFO, WARNING, ERROR, CRITICAL) |
| modelLoader.filesPath | object | `{"model_loader_py":"files/model_loader/model_loader.py","requirements_txt":"files/model_loader/requirements.txt"}` | Path on the host where model_loader.py and requirements.txt are located |
| modelLoader.resources.limits.cpu | string | `"200m"` |  |
| modelLoader.resources.limits.memory | string | `"256Mi"` |  |
| modelLoader.resources.requests.cpu | string | `"100m"` |  |
| modelLoader.resources.requests.memory | string | `"128Mi"` |  |