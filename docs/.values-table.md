...
| scaleFromZero | object | `{"admissionImage":"python:3.11-slim","enabled":false,"holdMinReplicasSeconds":300,"readyTimeoutSeconds":300}` | Start the first GPU Triton from Envoy when a client sends RepositoryIndex. SuperSONIC feature (not a KEDA ScaledObject field). Requires keda.enabled, keda.minReplicaCount: 0, and envoy.enabled. A successful index is returned only after Envoy has a healthy Triton upstream, so ModelConfig and inference can keep their normal (short) client timeouts. |
| scaleFromZero.readyTimeoutSeconds | int | `300` | How long Envoy waits for a healthy Triton upstream before rejecting RepositoryIndex |
| scaleFromZero.holdMinReplicasSeconds | int | `300` | Keep minReplicaCount at 1 this long after the last index so KEDA does not scale to 0 during warmup |
| scaleFromZero.admissionImage | string | `"python:3.11-slim"` | Image for the admission sidecar running next to Envoy |
| triton.replicas | int | `1` | Number of Triton server instances (if autoscaling is disabled). Ignored when scaleFromZero.enabled is true. |
...
| envoy.rate_limiter.prometheus_based.luaConfig | string | `"cfg/envoy-filter.lua"` | Lua filter used by the prometheus rate limiter and by scale-from-zero |
...
| keda.minReplicaCount | int | `1` | Minimum and maximum number of Triton servers. Use minReplicaCount: 0 with scaleFromZero.enabled to park GPUs when idle. |
| keda.maxReplicaCount | int | `2` |  |
| keda.pollingInterval | int | `30` | How often KEDA polls Prometheus for 1→N scaling |
| keda.cooldownPeriod | int | `120` | Period to wait after the last trigger is inactive before scaling to minReplicaCount |
| keda.zeroIdleReplicas | bool | `false` | If set to true, the server will release all GPUs when idle. Be careful: if the scaling metric is extracted from Triton servers, it will be unavailable, and scaling from 0 to 1 will never happen. Prefer scaleFromZero instead of this flag. |
